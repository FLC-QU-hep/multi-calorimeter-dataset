...
#SBATCH --array=0-999
#SBATCH --job-name=siw_100gev
#SBATCH --output=log/par04_siw_100gev/inline_%A_%a.out
#SBATCH --error=log/par04_siw_100gev/inline_%A_%a.err

# ============================================================================
# Par04_SiW ECal inline: simulate → ROOT → H5 → delete ROOT
...
export PAR04_COMPACT=$WORK_DIR/ddfastsim/install/Detector/xml/Par04_fullsim.xml

STEERING=$WORK_DIR/ddfastsim/options/Par04/Par04_ddsim_steer_steps_lcio.py
...
# --- Auto-chain second batch (OFFSET=1000, array 0-499) + postprocess ---
if [ $OFFSET -eq 0 ] && [ $SLURM_ARRAY_TASK_ID -eq 999 ]; then
    echo "Auto-submitting second batch OFFSET=1000 (jobs 1000-1499)"
    BATCH2=$(sbatch --parsable --array=0-499 --export=ALL,OFFSET=1000 $BASE_PATH/run/lemurs/$DET/simulation/submit_inline_100gev.sh)
    echo "Second batch job: $BATCH2"
    POSTPROC=$(sbatch --parsable --dependency=afterok:${BATCH2} $BASE_PATH/run/lemurs/$DET/simulation/postprocess_siw_150k.sh)
    echo "Postprocess job: $POSTPROC (depends on $BATCH2)"
fi
