...
#SBATCH --job-name=odd_postproc
#SBATCH --output=log/odd_100gev/postprocess_%j.out
#SBATCH --error=log/odd_100gev/postprocess_%j.err

# ============================================================================
# ODD 150k post-processing: merge → AllShowers finetune → PointCountFM training
# ============================================================================
...
# --- Step 1: Merge 1500 h5 files → odd_150k.h5 ---
echo "=== Step 1: Merging h5 files ==="
python3 scripts/lemurs/merge_h5_lemurs.py \
    --step merge-detector \
    --detector odd \
    --h5-subdir h5_150k_100gev \
    --output-name odd_150k.h5

MERGED=$BASE_PATH/output_dataset/ODD/odd_150k.h5
if [ ! -f "$MERGED" ]; then
...
mkdir -p $ALLSHOWERS/finetune_odd
cp $MERGED $ALLSHOWERS/finetune_odd/odd_110k.h5
echo "Copied to $ALLSHOWERS/finetune_odd/odd_110k.h5"

# --- Step 3: Create conditioning file (10k) ---
echo "=== Step 3: Creating odd_10k_cond.h5 ==="
python3 scripts/lemurs/create_cond.py \
    --detector odd \
    --src $MERGED \
    --n 10000 \
    --output finetune_odd/odd_10k_cond.h5

if [ ! -f "$ALLSHOWERS/finetune_odd/odd_10k_cond.h5" ]; then
    echo "FAILED: conditioning file not created"
    exit 2
fi
echo "Created: $(ls -lh $ALLSHOWERS/finetune_odd/odd_10k_cond.h5 | awk '{print $5}')"

# --- Step 4: Create PointCountFM training file ---
echo "=== Step 4: Creating PointCountFM odd_finetune_150k.h5 ==="
python3 -c "
...
if [ ! -f "$PCFM/odd_finetune_150k.h5" ]; then
    echo "FAILED: PointCountFM training file not created"
    exit 3
fi
echo "Created: $(ls -lh $PCFM/odd_finetune_150k.h5 | awk '{print $5}')"

echo "=========================================="
echo "All post-processing complete!"
echo "  Merged:     $MERGED"
echo "  AllShowers: $ALLSHOWERS/finetune_odd/odd_110k.h5"
echo "  Cond:       $ALLSHOWERS/finetune_odd/odd_10k_cond.h5"
echo "  PointCountFM: $PCFM/odd_finetune_150k.h5"
echo "=========================================="
