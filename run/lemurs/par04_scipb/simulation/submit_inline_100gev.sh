...
#SBATCH --array=0-999
#SBATCH --job-name=scipb_100gev
#SBATCH --output=log/par04_scipb_100gev/inline_%A_%a.out
#SBATCH --error=log/par04_scipb_100gev/inline_%A_%a.err

# ============================================================================
# Par04_SciPb ECal inline: simulate → ROOT → H5 → delete ROOT
...
if [ $OFFSET -eq 0 ] && [ $SLURM_ARRAY_TASK_ID -eq 999 ]; then
    ...
    POSTPROC=$(sbatch --parsable --dependency=afterok:${BATCH2} $BASE_PATH/run/lemurs/$DET/simulation/postprocess_scipb_150k.sh)
